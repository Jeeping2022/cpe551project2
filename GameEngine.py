# Authors: Christian O'Connell + Jason Pinga
...
class GameEngine:
    ...
    def initVeggies(self):
        ...
        while (count < NUMBEROFVEGGIES):
            ...
            if (self.__field[randPos % fieldWidth][randPos // fieldWidth] == None):
                self.__field[randPos % fieldWidth][randPos // fieldWidth] = self.__vegetables[
                    random.randint(0, len(self.__vegetables) - 1)]
    ...
    def moveRabbits(self):
        ...
        for rab in self.__rabbits:
            choices = [-1,0,1]
            xmove = random.choice(choices)
            ymove = random.choice(choices)
            newxpos = rab.getxpos() + xmove
            newypos = rab.getypos() + ymove
            if (newxpos < 0 or newxpos >= fieldWidth - 1):
                continue  # out of bounds, move is skipped
            elif (newypos < 0 or newypos >= fieldHeight - 1):
                continue  # out of bounds, move is skipped
    ...
    def intro(self):
        ...
        print("Rabbit:", self.__rabbits[0].getsymbol())
    ...
    def highScore(self):
        ...
        infile = open(HIGHSCOREFILE, "rb")
        hsp = []
        try:
            hsp = pickle.load(infile)
        except EOFError:
            makescodeworkgood = 0
            #gives eoferror if nothing is in the file
        infile.close()
        ...
        for x in hsp:
            print(x[0], ":", x[1])
        outfile = open(HIGHSCOREFILE, "wb")
        pickle.dump(hsp, outfile)
        outfile.close()
